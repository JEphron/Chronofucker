...
import time, sys, images2gif, random, os, gc
...
def worker(input_queue, output_queue):
    ...
    for x in range(width): # yay exponential time!
        ...
        for y in range(height):
            pixels[x].append([])
            append =  pixels[x][y].append # an optimization which shaves a few ms off the final time
    ...
    for x in range(width):
        for y in range(height):
            pixels[x][y] = sorted(pixels[x][y], reverse=True)
    ...
    print('processed chunk',chunk.id,'in', elapsed,'seconds!')
...
def process_chunks(chunks, should_reverse_chunks):
    ...
    out_queue = Queue()
    gc.disable() # provides a slight speed increase
    for chunk in chunks:
        p = Process(target=worker, args=(in_queue,out_queue))
        print("initializing processor:", chunk.id)
        p.start()
        workers.append(p)
    ...
    # random.shuffle(processed) # fun times
